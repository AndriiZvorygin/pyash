...
- `pya_to_json.mjs <path> [--memory-only] [--pretty]` — interpret a `.pya` file and print JSON (`memory`, `sandpits`, and `index` by `su name`).
...
- `work_supervisor.mjs [--world <path>] [--repository <path>]` — manage durable Sol/Luna work items. Use `background --watch` for a live human-readable run view, `report <task-id>` to regenerate a completed task report, and `--email-report <address>` to submit that same report through the configured mail transport.
