...
function titleScore({ item, title }, paraText) {
  ...
  if (item) {
    score += cueStrength(item, paraNorm);
  }
...
function sectionCueRegex(section) {
  ...
  const cues = [];
  if (item === '2' || title.includes('call for additional business')) {
    cues.push('\\bcall\\s+for\\s+additional\\s+business\\b');
  }
  if (item === '3' || title.includes('declarations of interest')) {
    cues.push('\\bdeclarations?\\s+of\\s+interest\\b');
  }
...
function parseAgendaSections(agendaText) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    // skip housekeeping sections where transcript matching is noisy
    if (/^(BY-LAWS|ADJOURNMENT|MATTERS POSTPONED|NOTICES OF MOTION)/iu.test(title)) continue;
...
async function ollamaPickParagraph({ section, startIndex, paragraphs, topCandidates, searchEnd }) {
  const candidates = topCandidates.slice(0, 12).map((c) => ({
    idx: c.index,
    score: Number(c.score.toFixed(3)),
    text: buildSnippet(paragraphs[c.index], 240)
  }));
  if (!candidates.length) {
    const end = Math.min(paragraphs.length, searchEnd);
    for (let i = startIndex; i < end && candidates.length < 12; i += 1) {
      candidates.push({
        idx: i,
        score: 0,
        text: buildSnippet(paragraphs[i], 220)
      });
    }
  }
  ...
  return { index, reason: String(parsed?.reason || '').slice(0, 180) };
}
...
function buildSectionStarts({ sections, chosen, paragraphCount }) {
  ...
  // If the first sections are unmatched, start from transcript beginning.
  ...
  // Force monotonic section order.
  for (let k = 1; k < starts.length; k += 1) {
    if (starts[k] <= starts[k - 1]) {
      starts[k] = Math.min(paragraphCount - 1, starts[k - 1] + 1);
    }
  }
  return starts;
}

async function ollamaRefineSectionWindow({ section, nextSection, paragraphs, minStart, maxEnd }) {
  ...
  if (!Number.isFinite(start) || !Number.isFinite(end)) {
    return { start: lo, end: lo, reason: 'unparseable-json' };
  }
...
function toSeriesText(chips) {
  ...
  for (let i = 0; i < chips.length; i += 1) {
    ...
    lines.push(`su name wise chip ${n} ob text ${text} ya`);
  }
  lines.push('prah');
  return `${lines.join('\n')}\n`;
}
...
async function main() {
  ...
  const paragraphs = splitParagraphs(readText(plainPath));
  if (!paragraphs.length) throw new Error('plain transcript has no paragraphs');

  const sections = parseAgendaSections(readText(agendaPath));
  ...
  for (let i = 0; i < sections.length; i += 1) {
    ...
    process.stdout.write(`[agenda-wise] atindex num ${i + 1} toindex num ${sections.length} item ${sec.item} method ${method} paragraph ${finalIndex} score ${confidence.toFixed(3)}\n`);
  }

  const starts = buildSectionStarts({
    sections,
    chosen,
  ...
  for (let i = 0; i < boundaries.length; i += 1) {
    ...
    if (hardItem === '8' && forumStart >= 0) {
      ...
      process.stdout.write(`[agenda-wise] refine atindex num ${i + 1} toindex num ${boundaries.length} item ${boundaries[i].item} since ${boundaries[i].start} until ${boundaries[i].end} method ${boundaries[i].method}\n`);
      continue;
    }
    ...
    if (i > 0 && boundaries[i].start <= boundaries[i - 1].end) {
      boundaries[i].start = Math.min(paragraphs.length - 1, boundaries[i - 1].end + 1);
    }
  ...
  process.stdout.write(`[agenda-wise] sections matched: ${chosen.length}/${sections.length}\n`);
  process.stdout.write(`[agenda-wise] wise chips: ${chips.length}\n`);
  process.stdout.write(`[agenda-wise] matches: ${matchPath}\n`);
}
