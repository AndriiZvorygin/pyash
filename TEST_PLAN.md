...
## What is covered
- Parser: quoted text and blocks, short aliases (`su/ob`), compositional keyword mapping (fromtext/during/become/totext/as), minimal declaratives, bare names.
- Core interpreter: declaratives, last-write wins, add imperative, giant conditional, topic sugar, def mood, storing `do` commands and result facts, bare add without explicit target.
...
- Motor: Ollama HTTP client streaming mock tests.
...
- More REPL-level integration tests (typing sentences and inspecting memory output order).
- Error-path tests for missing handlers or malformed inputs in dynamic verbs.
...
Tests use Node’s built-in `node:test` with `assert/strict`; keep new tests close to the behaviour they cover.
