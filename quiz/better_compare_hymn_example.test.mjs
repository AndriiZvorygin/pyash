...
test("better compare hymn from filename keeps draft artifacts and writes winner copies", async () => {
  ...
  assert.match(text, /ob name text hook_hint be equally from text "" then/);
  assert.match(text, /su name hymn draft run stage from text of ob of this with text of ob of hook_hint to name text manuscript out be hymn manuscript do/);
