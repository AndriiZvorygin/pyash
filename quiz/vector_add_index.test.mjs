...
test("plus to vector element by index mutates in place", async () => {
  forget();
  await interpret(parse("exists su name values ob ve num 1 2 3 be vector ya"));
  await interpret(parse("ob name values from num 5 at num 2 be plus do"));

  const vec = remember("values");
  assert.deepEqual(vec?.ob?.ve?.values, [1, 7, 3]);
});

test("plus using ob num and from vec at index mutates in place", async () => {
  forget();
  await interpret(parse("exists su name values ob ve num 10 20 30 be vector ya"));
  await interpret(parse("ob num 3 from name values at num 1 be plus do"));
