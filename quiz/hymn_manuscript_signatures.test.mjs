...
test("hymn manuscript module keeps educational staged flow with verify platform", async () => {
  ...
  assert.match(text, /exists su name hymn hook chooser prompt ob text quoted\.text\.Choose the single best hook candidate/);
  ...
  assert.match(text, /exists su name hymn hook chooser mind be mind fromtext name hymn hook chooser prompt ya/);
  ...
  assert.match(text, /Line 2 should be a very short echo or extension \(2 to 4 words\)\./);
  assert.match(text, /Line 4 should be a very short echo or extension \(2 to 4 words\)\./);
  assert.match(text, /Build the chorus around one short refrain rather than four separate full ideas\./);
  assert.match(text, /Do not restate Verse 1 as long prose-like lines\./);
  assert.match(text, /Keep each line punchy and compact; if a line starts getting long, rewrite it shorter\./);
  assert.match(text, /Make lines 2-4 brief echoes or extensions of the same refrain, not new subtopics\./);
  ...
  assert.match(text, /su name hymn chorus checks be series def[\s\S]*?su name word_min ob num 8 ya[\s\S]*?su name word_max ob num 22 ya/);
