...
test("runjs hash mismatch surfaces be error ya", async () => {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "pyash-exchange-hash-js-"));
  ...
  await execFileAsync(scriptPath, [
    "--newspaper",
    "--run-id", "run-exchange-hash-js",
    "--run-time", "2025-01-01T00:00:00Z",
    programPath
  ], { cwd: tmpDir, timeout: 120000 }).catch(() => {});

  const newspaperPath = path.join(tmpDir, "newspaper", "run-exchange-hash-js.pya");
