...
test("yaml->pyash golden example (interpret)", async () => {
  forget();
  const entryPath = path.resolve("examples/pyash/yaml-to-pyash-golden.pya");
...
test("yaml->pyash golden example (compiled JS)", async () => {
  forget();
  const sentence = parse("from filename \"examples/pyash/yaml-to-pyash-golden.pya\" from state pyash to state javascript to text output be compile do");
...
test("yaml->pyash golden example (compiled C)", async () => {
  forget();
  const sentence = parse("from filename \"examples/pyash/yaml-to-pyash-golden.pya\" from state pyash to state c to text output be compile do");
  const result = await interpret(sentence);
  const wrapped = result?.ob?.text ?? result?.value?.text ?? "";
  const c = unwrapQuoted(wrapped, "c");

  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "pyash-yaml-golden-"));
