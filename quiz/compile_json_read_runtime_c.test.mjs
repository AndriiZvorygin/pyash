...
test("compile runtime read json state to C matches interpreter map defs", async () => {
  forget();

  const json = JSON.stringify({
    title: "Ledger",
    entries: [{ total: 1.5 }, { total: 2.25 }]
  });

  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "pyash-json-read-"));
  const interpOut = path.join(tmpDir, "interp.txt");
  const cOut = path.join(tmpDir, "c.txt");

  const interpreterLines = [
    `ob text ${JSON.stringify(json)} from state json to name ledger be read do`,
    `ob name ledger be write to filename "${interpOut}" do`
  ];
  ...
  const compiledLines = [
    `ob text ${JSON.stringify(json)} from state json to name ledger be read do`,
    `ob name ledger be write to filename "${cOut}" do`
  ];
