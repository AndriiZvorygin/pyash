...
test("giant conditional controls next statement with inline values", async () => {
  ...
  await run("exists su name collector ob num 1 be number ya");
  ...
  assert.equal(res, "exists su name collector ob num 3 be number ya");
  ...
  assert.equal(res2, "exists su name collector ob num 1 be number ya");
});
...
test("giant compares su against literal", async () => {
  ...
  const res = await run("su name collector ob what que");
  assert.equal(res, "exists su name collector ob num 9 be number ya");
});
...
test("giant compares su against another su value", async () => {
  ...
  assert.equal(res, "exists su name lhs ob num 7 be number ya");
...
test("giant errors when exists su name is unknown", async () => {
  forget();
