...
test("multiply using named operand and target", async () => {
  ...
  assert.equal(acc.ob.num, 10, "target should be updated with product");
  assert.equal(result.ob.num, 10, "result fact should mirror product");
});
...
test("divide using by for divisor and update target", async () => {
  ...
  assert.equal(acc.ob.num, 5, "target should hold quotient");
  assert.equal(result.ob.num, 5, "result fact should mirror quotient");
});

test("divide by zero throws", async () => {
  forget();

  await assert.rejects(() => run("ob num 1 by num 0 be divide do"), /by cannot be zero/);
});
...
test("missing operands or by triggers signature errors", async () => {
  ...
  await assert.rejects(() => run("by num 2 be multiply do"), /ob or from is required/);
  await assert.rejects(() => run("ob num 2 be multiply do"), /by is required/);
  await assert.rejects(() => run("ob num 2 be divide do"), /by is required/);
});
