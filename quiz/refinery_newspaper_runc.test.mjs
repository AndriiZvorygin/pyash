...
test("runc refinery newspaper records platform evoke/result", async () => {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "pyash-refinery-news-c-"));
  ...
  await execFileAsync(scriptPath, [
    "--newspaper",
    "--refinery", "line",
    "--run-id", "run-refinery-c",
    "--run-time", "2025-01-01T00:00:00Z",
    programPath
  ], { cwd: tmpDir, timeout: 120000 });

  const newspaperPath = path.join(tmpDir, "newspaper", "run-refinery-c.pya");
