...
test("pure verifier module matches builtin output shape", async () => {
  ...
  await run('from name ./module/verifier_circle.pya to name verifier circle module be import do');
  await run('exists su name pure task ob text "Task." be text ya');

  await run('ob name text pure task for name text author by name text verifier atleast num 0.8 atmost num 3 to name text pure result be verifier circle module verifier trying do');
  const pureResult = remember("pure result")?.ob?.text;

  forget();
  await setupRetryFixtures();
  await run('ob text "Task." for name author by name verifier atleast num 0.8 atmost num 3 to name text builtin result be verify loop do');
...
test("pure verifier module can return map produce via genitives", async () => {
  forget();
  await setupRetryFixtures();
  await run('from name ./module/verifier_circle.pya to name verifier circle module be import do');
  await run('exists su name pure task ob text "Task." be text ya');

  await run('ob name text pure task for name text author by name text verifier atleast num 0.8 atmost num 3 to name map produce be verifier circle module verifier trying do');
...
test("pure verifier matches builtin score parsing behavior", async () => {
  forget();
  await run('from name ./module/verifier_circle.pya to name verifier circle module be import do');
  await setupScoreRetryCeremonies();
  await run('exists su name pure score task ob text "Task." be text ya');

  await run('ob name text pure score task for name text author by name text verifier atleast num 0.95 atmost num 1 to name map produce be verifier circle module verifier trying do');
  ...
  forget();
  await setupScoreRetryCeremonies();
  await run('ob text "Task." for name author by name verifier atleast num 0.95 atmost num 1 to name text builtin result be verify loop do');
  const builtinResult = remember("builtin result")?.ob?.text;
...
test("pure verifier matches builtin last-line-only verdict behavior", async () => {
  forget();
  await run('from name ./module/verifier_circle.pya to name verifier circle module be import do');
  await setupLastLineCeremonies();
  await run('exists su name pure line task ob text "Task." be text ya');

  await run('ob name text pure line task for name text author by name text verifier atleast num 0.8 atmost num 3 to name map produce be verifier circle module verifier trying do');
  const pureProduce = remember("produce");
  const pureResult = pureProduce?.ob?.map?.result?.text ?? pureProduce?.ob?.map?.result?.ob?.text ?? "";
  const pureDecision = pureProduce?.ob?.map?.decision?.text ?? pureProduce?.ob?.map?.decision?.ob?.text ?? "";
