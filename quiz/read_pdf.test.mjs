...
test("read pdf extracts text", { skip: !pdftotextAvailable }, async () => {
  forget();
  const filename = "/tmp/pyash-read-pdf.pdf";
  const source = "Hello PDF";
  await fs.writeFile(filename, buildPdf({ text: source }));

  await run("ob name read from filename \"./module/read_pdf.pya\" to name read be import do");
  await run(`from filename \"${filename}\" fromstate wo pdf to name text out be read do`);
