...
test("runc can emit tool event for command", async () => {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "pyash-newspaper-command-c-"));
  ...
  const newspaperPath = path.join(tmpDir, "newspaper", "run-command-c.pya");
