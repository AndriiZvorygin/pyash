...
test("runjs refinery reruns stage when checkpoint output file is missing", async () => {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "pyash-refinery-checkpoint-js-"));
  ...
  const runjsPath = path.join(repoRoot, "runjs");
  await execFileAsync(runjsPath, [
    "--newspaper",
    "--refinery", "line",
    "--run-id", "run-checkpoint-js",
