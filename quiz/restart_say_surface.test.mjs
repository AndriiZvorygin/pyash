...
test("restart as wo say interrupts queue and frees comfyui when say backend is comfyui", async () => {
  forget();
  ...
  globalThis.fetch = async (url, opts = {}) => {
    ...
    if (String(url).endsWith("/free")) return { ok: true, status: 200, statusText: "OK", async json() { return {}; } };
    throw new Error(`unexpected url ${url}`);
  };
  try {
    const out = await interpret(parse("be restart as wo say do"));
    ...
    assert.equal(calls.filter(c => c.url.endsWith("/free")).length, 1);
  } finally {
