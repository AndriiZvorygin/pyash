...
test("compile module behavior example to C and run", async () => {
  forget();

  const entryPath = path.resolve("examples/pyash/module-import-behavior.pya");
  const sentence = parse(`from filename "${entryPath}" to state c to text output be compile do`);
