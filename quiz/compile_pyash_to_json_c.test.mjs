...
test("compile pyash map def chain to canonical json and run (c)", async () => {
  forget();

  const sentence = parse(`from filename "examples/pyash/pyash-to-json-canonical.pya" from state pyash to state c to text output be compile do`);
  const result = await interpret(sentence);
  const wrapped = result?.ob?.text ?? result?.value?.text ?? "";
  const c = wrapped.replace(/^\s*quoted\.c\.\s*/, "").replace(/\s*\.c\.quoted\s*$/, "");

  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "pyash-pyash-json-"));
