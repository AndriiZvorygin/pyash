...
test("module manuscript scored module exports checkpoint-first helper ceremonies", async () => {
  ...
  assert.match(moduleSource, /to history of nextstate be text do/u);
  assert.match(moduleSource, /be giant from num of best score of with of this then/u);
  assert.match(moduleSource, /to best attempt of nextstate be text do/u);
  assert.match(moduleSource, /to best score of nextstate be plus do/u);
