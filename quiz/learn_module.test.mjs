...
test("learn module exports text and filename learning ceremonies", async () => {
  ...
  assert.match(text, /Only include a concrete claim when it is explicitly supported by chunk cards\./u);
  assert.match(text, /If support is uncertain, omit the claim instead of guessing\./u);
  assert.match(text, /Do not add quotations, scripture references, named entities, or specific examples that do not appear in chunk cards\./u);
  assert.match(text, /Keep section item counts inside schema ranges exactly:/u);
  ...
  assert.match(text, /Do not introduce new entities, references, examples, or scenes that are absent from the draft card\./u);
  assert.match(text, /If support in the draft card is weak or ambiguous, keep wording conservative and shorter rather than expanding\./u);
  ...
  assert.match(text, /learning merge prompt for name mind to name text teaching raw by num 0 atmost num 1800 be write do/u);
  assert.match(text, /learning refine prompt for name mind to name text teaching raw by num 0 atmost num 1800 be write do/u);
  ...
  assert.match(text, /fromtext name learning merge prompt for name mind to name text teaching raw by num 0 atmost num 1800 be write do/u);
  ...
  assert.match(text, /fromtext name learning refine prompt for name mind to name text teaching raw by num 0 atmost num 1800 be write do/u);
  ...
  assert.equal((text.match(/su name mind discharge stage as wo mind be discharge do/gu) ?? []).length, 1);
