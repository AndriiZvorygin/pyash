...
test("csv->pyash golden example (interpret)", async () => {
  forget();
  const entryPath = path.resolve("examples/pyash/csv-pyash.pya");
...
test("csv->pyash golden example (compiled JS)", async () => {
  forget();
  const sentence = parse("from filename \"examples/pyash/csv-pyash.pya\" from state pyash to state javascript to text output be compile do");
...
test("csv->pyash golden example (compiled C)", async () => {
  forget();
  const sentence = parse("from filename \"examples/pyash/csv-pyash.pya\" from state pyash to state c to text output be compile do");
  const result = await interpret(sentence);
  const wrapped = result?.ob?.text ?? result?.value?.text ?? "";
  const c = wrapped.replace(/^\s*quoted\.c\.\s*/, "").replace(/\s*\.c\.quoted\s*$/, "");

  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "pyash-csv-golden-"));
