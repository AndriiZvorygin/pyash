...
test("subordinate clause parses into la sentence object", () => {
  const sentence = parse("ob la exists su name clause ob name example ko be evoke ya");
...
test("subordinate clause allows mood inside", () => {
  const sentence = parse("ob la exists su name clause ob name example be text ya ko be evoke ya");
...
test("subordinate clause formats with la/ko", () => {
  ...
  assert.match(rendered, /la exists su name clause ob name example ko/);
});
...
test("sentence splitter ignores moods inside clauses", () => {
  const text = "ob la exists su name clause be text ya ko be evoke ya exists su name next be text ya";
  const sentences = splitSentences(text);
  assert.equal(sentences.length, 2);
  assert.equal(sentences[0], "ob la exists su name clause be text ya ko be evoke ya");
});
