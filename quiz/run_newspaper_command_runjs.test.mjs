...
test("runjs can emit tool event for command", async () => {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "pyash-newspaper-command-js-"));
  ...
  const newspaperPath = path.join(tmpDir, "newspaper", "run-command-js.pya");
