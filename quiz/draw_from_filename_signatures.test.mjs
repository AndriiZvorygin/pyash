...
test("draw from filename module writes artifacts and know produce outputs", async () => {
  ...
  assert.match(text, /ob num of width of draw size thumbnail to name draw cmd base be plus do/);
  assert.match(text, /ob text " --height \\""/);
  assert.match(text, /ob num of height of draw size thumbnail to name draw cmd base be plus do/);
