...
const expectedJson = "{\"age\":36,\"flags\":[true,false],\"meta\":{\"active\":true},\"name\":\"Ada\",\"pets\":[{\"kind\":\"cat\"},{\"kind\":\"dog\"}]}";
const expectedPyash = [
...
test("pyash->json->pyash roundtrip (interpret)", async () => {
  forget();
  const entryPath = path.resolve("examples/pyash/pyash-json-pyash-roundtrip.pya");
  ...
  assert.equal(logs[0], expectedJson);
  assert.deepEqual(normalizePyash(logs[1]), expectedPyash);
});

test("pyash->json->pyash roundtrip (compiled JS)", async () => {
  forget();
  const sentence = parse("from filename \"examples/pyash/pyash-json-pyash-roundtrip.pya\" from state pyash to state javascript to text output be compile do");
...
test("pyash->json->pyash roundtrip (compiled C)", async () => {
  forget();
  const sentence = parse("from filename \"examples/pyash/pyash-json-pyash-roundtrip.pya\" from state pyash to state c to text output be compile do");
  const result = await interpret(sentence);
  const wrapped = result?.ob?.text ?? result?.value?.text ?? "";
  const c = wrapped.replace(/^\s*quoted\.c\.\s*/, "").replace(/\s*\.c\.quoted\s*$/, "");

  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "pyash-roundtrip-"));
  ...
  const jsonLine = firstBreak >= 0 ? stdout.slice(0, firstBreak) : stdout.trim();
  const rest = firstBreak >= 0 ? stdout.slice(firstBreak + 1) : "";

  assert.equal(jsonLine.trim(), expectedJson);
