...
test("exchange reuses artifact name for repeated writes to same locator", async () => {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "pyash-exchange-reuse-"));
  ...
  await execFileAsync("node", [
    runPath,
    "--newspaper",
    "--run-id", "run-exchange-reuse",
    "--run-time", "2025-01-01T00:00:00Z",
    programPath
  ], { cwd: tmpDir, timeout: 120000 });

  const newspaperPath = path.join(tmpDir, "newspaper", "run-exchange-reuse.pya");
