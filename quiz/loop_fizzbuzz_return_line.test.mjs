...
test("loop ceremony can return fizzbuzz line and write outside", async () => {
  ...
  await interpret(parse("su name fizzbuzz line by num 0 to name text line be ceremony def"));
  await interpret(parse("exists su name line ob text quoted.text..text.quoted be text ya"));
  ...
  await interpret(parse("su name fizzbuzz write fromindex num 0 toindex num 0 be ceremony def"));
  await interpret(parse("exists su name line ob text quoted.text..text.quoted be text ya"));
  await interpret(parse("by num of fromindex of this to name line be fizzbuzz line do"));
  await interpret(parse("ob name line be write do"));
  await interpret(parse("su name fizzbuzz write be ceremony prah"));
