...
export function renderWorkTaskReport(task) {
  ...
  const lines = [
    ...
    `Domain: ${text(current.domain) || "(none)"}`,
    `Source: ${text(current.source?.identity) || "(none)"}`,
    `Deadline: ${text(current.deadline) || "(none)"}`,
    `Dependencies: ${list(current.dependencies).join(", ") || "(none)"}`,
    `Delegated by: ${text(current.delegatedBy) || "(none)"}`,
    `Escalation: ${text(current.escalation?.state) || "(none)"}${text(current.escalation?.target) ? ` -> ${text(current.escalation.target)}` : ""}`,
    `Delegation events: ${list(current.delegationEvents).length || 0}`,
    "",
    "Manager:",
