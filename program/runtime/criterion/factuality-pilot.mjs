...
export function normalizeNativeJudgeResponse(raw, { maxClaims = CLAIM_LIMIT } = {}) {
  ...
  const explanationValue = evaluation.explanation;
  const explanation = typeof explanationValue === "object" ? "" : text(explanationValue ?? native.Analysis_process ?? native.reasoning);
  const criterion = evaluation.criterion ?? native.criterion ?? (explanationValue && typeof explanationValue === "object" ? explanationValue : parseCriterionPayload(explanation));
...
export async function runMeetingBankFactualityPilot({
  ...
  const selectedIdentity = new Set(models.flatMap(model => selection.map(sample => `${model}\u0000${sample.id}`)));
  const resultRows = [...rows.values()].filter(row => selectedIdentity.has(row.identity));
  const selectedGenerationRows = generationRows.filter(row => models.includes(row.model) && selection.some(sample => String(sample.id) === String(row.sampleId)));
  ...
  const finalRun = await writeRunArtifacts({
    ...
    generationStats: { attempted: selectedGenerationRows.length, successful: selectedGenerationRows.filter(row => row.status === "ok").length, transientRetries: selectedGenerationRows.reduce((sum, row) => sum + Number(row.metrics?.transportRetries ?? 0), 0), permanentFailures: selectedGenerationRows.filter(row => row.status === "error").length }, judgeStats,
