...
async function runCandidateVerifier({ candidateText, verifierName, interpret }) {
  ...
  try {
    ...
    verdict = String(fact?.ob?.text ?? "").trim();
  } catch (err) {
    failed = true;
  } finally {
    popMemoryContext();
  ...
  return verdict.toUpperCase().includes("FAIL");
}

async function errorSieve(sentence) {
  ...
  while (changed && attempts < maxAttempts) {
    ...
    for (let i = 0; i < chunks.length; i += 1) {
      ...
      const stillFails = await verify(candidateText);
      if (stillFails) {
...
export const signatures = [
  { signatureWords: ["be", "error", "sieve", "ob", "text", "to", "name", "text"], handler: errorSieve },
  { signatureWords: ["be", "error", "sieve", "ob", "name", "text", "to", "name", "text"], handler: errorSieve },
  { signatureWords: ["be", "error", "sieve", "ob", "text", "from", "name", "to", "name", "text"], handler: errorSieve },
  { signatureWords: ["be", "error", "sieve", "ob", "name", "text", "from", "name", "to", "name", "text"], handler: errorSieve },
  { signatureWords: ["be", "error", "sieve", "ob", "text", "atmost", "num", "to", "name", "text"], handler: errorSieve },
  { signatureWords: ["be", "error", "sieve", "ob", "name", "text", "atmost", "num", "to", "name", "text"], handler: errorSieve },
  { signatureWords: ["be", "error", "sieve", "ob", "text", "from", "name", "atmost", "num", "to", "name", "text"], handler: errorSieve },
  { signatureWords: ["be", "error", "sieve", "ob", "name", "text", "from", "name", "atmost", "num", "to", "name", "text"], handler: errorSieve }
];

export default errorSieve;
