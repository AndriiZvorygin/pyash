function exchangeRuntimeHelper() {
  return [
    ...
    "}",
    "let pyaCommandCounter = 0;",
    "function pyaNextCommandRequestId() {",
    "  pyaCommandCounter += 1;",
    "  return `command request ${String(pyaCommandCounter).padStart(6, \"0\")}`;",
    "}",
    ...
    "}",
    "function pyaResolveAgentPath(target) {",
    "  const agentCwd = pyaResolveAgentCwd();",
    "  const raw = String(target ?? \"\");",
    "  if (!agentCwd) return { resolved: path.resolve(raw), agentCwd: null, outside: false };",
    "  const resolved = path.isAbsolute(raw) ? path.resolve(raw) : path.resolve(agentCwd, raw);",
    ...
    "  return rel;",
    "}",
    "function pyaRecordArtifact(locator, bytes, op, requestIdentity) {",
    ...
    "    if (op) pyaEmitExchange(`exists su name ${existing}${requestIdentity ? ` ob name ${requestIdentity}` : \"\"} as name ${op} from name exchange be exchange ya`);",
    "    return existing;",
    "  }",
    ...
    "  const obText = requestIdentity ? `ob name ${requestIdentity}` : `ob text ${locatorText}`;",
    ...
    "  if (op) {",
    "    pyaEmitExchange(`exists su name ${name}${requestIdentity ? ` ob name ${requestIdentity}` : \"\"} as name ${op} from name exchange be exchange ya`);",
    "  }",
    "  return name;",
    "}",
    "function pyaReadTextFile(filename, op, requestIdentity) {",
    "  const buf = fs.readFileSync(filename);",
    "  pyaRecordArtifact(filename, buf, op, requestIdentity);",
    "  return buf.toString(\"utf8\");",
    "}",
    "function pyaWriteTextFile(filename, text, op, requestIdentity) {",
    "  const normalized = pyaNormalizeNewlines(text);",
    "  const resolvedInfo = pyaResolveAgentPath(filename);",
    "  if (resolvedInfo.outside) {",
    "    throw new Error(`write defective: outside agent cwd (${resolvedInfo.agentCwd})`);",
    "  }",
    "  const resolved = resolvedInfo.resolved;",
    "  fs.writeFileSync(resolved, normalized);",
    "  const buf = Buffer.from(String(normalized ?? \"\"), \"utf8\");",
    "  pyaRecordArtifact(resolved, buf, op, requestIdentity);",
    "}"
  ].join("\n");
}
