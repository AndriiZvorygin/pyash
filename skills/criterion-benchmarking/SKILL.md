...
Use the same `--profile` and `--context-length` for every model in a comparison. The normal profiles are `summary_direct` and `reasoning`; the default comparison models are `qwen3.5:9b` and `hf.co/empero-ai/Qwen3.8-9B-Distill-GGUF:Q4_K_M`.
