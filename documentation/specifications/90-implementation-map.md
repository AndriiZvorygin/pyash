...
| Tool envelope (tool events) | `program/command/run_pya_program.mjs` (`emitToolEvent`), `program/verbs/exchange/compile.mjs` (tool event emits), `program/verbs/exchange/helpers_c.mjs` (`pya_emit_exchange`) |
...
| Mind (compiled JS/C) | `program/verbs/exchange/compile.mjs` (mind runtime JS), `program/verbs/exchange/helpers_c.mjs` (`MIND_RUNTIME_HELPER`) |
| Refinery | `program/bridge/refinery.mjs`, `program/command/run_pya_program.mjs` (refinery runner + newspaper), compiled: `program/verbs/exchange/compile.mjs` |
