...
- Verified by gcc+run quizzes: scalars (`number`/`text`), `write`, `plus`/`subtract`/`remains` (`fmod`), `equally`/`tiny`/`giant` with `then`, loops (`fromindex`/`toindex`), and vectors (read/write + print).
