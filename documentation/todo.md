...
- Add error-handling paths for ceremonies/sandpits using `ret` with `be error`, and surface those in main memory/results [bounded interpreter/JavaScript ceremony and sandpit slice completed; C structured error ABI and CLI policy remain deferred].
...
- Ceremony/sandpit error propagation: `ret` with `be error` preserves canonical `be error do` carriers through nested compiled ceremonies and loops, then surfaces one truthful `be error ya` result at the top-level observation boundary; focused interpreter/JavaScript parity coverage and `examples/pyash/ceremony-error-propagation.pya` added. C structured error-return ABI, CLI exit status, and automatic compiled source-context enrichment remain deferred.
