...
- **Compile**: Verb turning Pyash text into parsed sentences + JSON via `program/program.mjs`; stores output in memory.
- **Read**: Verb that loads content (e.g., `program/verbs/read_from_filename.mjs`) and stores text.
- **Program**: `program/program.mjs` builds sentences/labels from plain text; used by compile.
