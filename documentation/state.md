...
- `program/verbs/`: dynamic dispatch (plus/subtract/read) and direct verbs (giant, tiny, equally, mind, understand). Mind pulls config from keyword roles (`as`, `accordingto`), read dispatches to handlers, understand builds program JSON.
...
## What works now
- All quizzes green (`node --test quiz`): parser quoting/text, compositional keyword normalization, mind config/use, read filename handler, understand paths, vector indexing and math, at-all mapping for vectors, plus/subtract/multiply/divide, remains, conditionals, translation/compile paths, moods `ya/def/do/ret` stored, sandpit execution for ceremonies.
- Imperatives store both the command and a result fact; bare plus (`ob num 3 to num 4 be plus do`) or subtract produces `result` with updated value in memory. `result` can be chained into subsequent calls.
